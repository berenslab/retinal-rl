...
def simulation_plot(
    ...
    ltntax.imshow(
        data, cmap="viridis", interpolation="nearest", aspect="auto", vmin=-1, vmax=1
    )
    ltntax.vlines(
        pos_col, 0, data.shape[0], color="grey", linewidth=0.3, linestyle="--"
    )
    ltntax.vlines(
        neg_col, 0, data.shape[0], color="black", linewidth=0.3, linestyle=":"
    )
    ...
    cbar_ax = fig.add_axes(
        [ltntax_pos.x1 + 0.07, ltntax_pos.y0 + 0.025, 0.005, ltntax_pos.height]
    )
...
def plot_acts_tsne_stim(sim_recs):  # plot sorted activations
    ...
    plt.imshow(
        data, cmap="viridis", interpolation="nearest", aspect="auto", vmin=-4, vmax=4
    )
    # plt.colorbar()
