...
class PrototypicalEncoder(NeuralCircuit):
    def __init__(
        self, latent_size: int, input_shape: list[int], act_name: str
    ):
        super().__init__()
...
class PrototypicalDecoder(NeuralCircuit):
    def __init__(
        ...
        deconv_layers = OrderedDict[str, nn.Module](
            [
                ("deconv1_filters", nn.ConvTranspose2d(64, 32, 3, stride=2, padding=1, output_padding=1)),
                ("deconv1_output", self.str_to_activation(self.act_name)),
                ("deconv2_filters", nn.ConvTranspose2d(32, 16, 3, stride=2, padding=1, output_padding=1)),
                ("deconv2_output", self.str_to_activation(self.act_name)),
                ("deconv3_filters", nn.ConvTranspose2d(16, 3, 3, stride=1, padding=1)),
                ("deconv3_output", nn.Sigmoid())  # Assuming output is an image scaled between 0 and 1
            ]
        )
