...
def dog_2d(
    ...
    a1 = (np.cos(theta) ** 2) / (2 * sigma1_x**2) + (np.sin(theta) ** 2) / (
        2 * sigma1_y**2
    )
    b1 = -(np.sin(2 * theta)) / (4 * sigma1_x**2) + (np.sin(2 * theta)) / (
        4 * sigma1_y**2
    )
    c1 = (np.sin(theta) ** 2) / (2 * sigma1_x**2) + (np.cos(theta) ** 2) / (
        2 * sigma1_y**2
    )

    a2 = (np.cos(theta) ** 2) / (2 * sigma2_x**2) + (np.sin(theta) ** 2) / (
        2 * sigma2_y**2
    )
    b2 = -(np.sin(2 * theta)) / (4 * sigma2_x**2) + (np.sin(2 * theta)) / (
        4 * sigma2_y**2
    )
    c2 = (np.sin(theta) ** 2) / (2 * sigma2_x**2) + (np.cos(theta) ** 2) / (
        2 * sigma2_y**2
    )

    g1 = amp1 * np.exp(
        -(a1 * (x - x0) ** 2 + 2 * b1 * (x - x0) * (y - y0) + c1 * (y - y0) ** 2)
    )
    g2 = amp2 * np.exp(
        -(a2 * (x - x0) ** 2 + 2 * b2 * (x - x0) * (y - y0) + c2 * (y - y0) ** 2)
    )
